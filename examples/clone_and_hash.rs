...
//! `Self`, `hash` is generic over the hasher). Listing them in the shim's
//! bounds is recognized and handled specially instead of passed through: the
//! generated `DynShape` shim carries hidden machinery, and only `Clone + Hash`
//! implementors of `Shape` receive the shim.
//!
...
//! - `Hash` is implemented for `dyn DynShape`, which also covers `&dyn
//! DynShape` and, through the standard library's forwarding impl, `Box<dyn
//! DynShape>`.
//!
...
fn main() {
    ...
    // Box<dyn DynShape> is Clone, so the whole Vec clones.
